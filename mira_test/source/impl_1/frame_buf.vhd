...
architecture architecture_frame_buf of frame_buf is
  ...
  -- how many words storage is needed. Note there are 4 pix per word
  constant N_WORDS          : integer := (N_COLS/4) * N_LINES/N_SLICES;
  -- Fixed 16 bit result per pixel. This limits the number of frames that can be summed without overflow risk.
  ...
  begin
    ...
    if SumCntxDP = 0 then               -- first pass
      for ipix in 0 to 3 loop
        MemInxD((ipix+1)*16-1 downto ipix*16)
          <= std_logic_vector(resize(unsigned(PixDatxDP((ipix+1)*BIT_DEPTH-1 downto ipix*BIT_DEPTH)), 16));
      end loop;
