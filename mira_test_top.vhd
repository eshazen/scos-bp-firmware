...
entity mira_test_top is
  port (
    Clk48xCI   : IN  std_logic;                            -- 48MHz oscillator input
    ResetxRI   : IN  std_logic;                            -- reset asynch active high
...
architecture architecture_mira_test_top of mira_test_top is
  ...
  signal ClkxC : std_logic;             -- PLL clock output (96MHz nominal)
  signal PLLLockxS : std_logic;         -- PLL lock (LED 13 indicates unlock)
  ...
  signal CfgRegxDP, CfgRegxDN : cfgreg_type; -- present and next CFG register values
  
  type cfgfsmsreg_type is array (0 to N_CMD_BYTES-1) of std_logic_vector(7 downto 0);
  signal CfgFSMSRegxDP, CfgFSMSRegxDN : cfgfsmsreg_type; -- shift register for command bytes
  
  type cfgfsmstate_type is (sIdle, sRxBytes, sDecodeCmd, sTxHeader, sTxReg);
  signal CfgStatexDP, CfgStatexDN : cfgfsmstate_type;

  signal CfgByteCntxDP, CfgByteCntxDN : integer range 0 to N_CMD_BYTES-1; -- header byte count for SPI cmds
  signal CfgPDatOutxD : std_logic_vector(7 downto 0);               -- SPI transmit for cfg R/W
  signal CfgPDatOutValidxS : std_logic;                             -- config data output valid
  signal RdyForCfgPDatxS : std_logic;                               -- config output Tx ready
  signal CfgPDatInxD : std_logic_vector(7 downto 0);                -- config input Rx data
  signal CfgPDatInValidxS : std_logic;                              -- config input data valid
